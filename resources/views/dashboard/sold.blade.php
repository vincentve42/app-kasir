 ...
 @endphp
  ...
  @endif
    ...
    <div class="p-6 sm:ml-64 " x-data="{productopen : true, addproduct : false, productbutton : true}">
        ...
        <div class="sm:mt-10 sm:w-390 sm:h-200 shadow-xl">
            
            <form x-show="addproduct" action="{{ route('AddProduct') }}" method="post">
                @csrf
                <h1 class="p-6 font-bold text-xl">Add Product</h1>
                <div class="pl-6 pt-2">
                    <input type="text" name="judul" placeholder="Judul" class="border-b border-black text-xl w-100">
                </div>
                   <div class="pl-6 pt-5">
                    <input type="text" name="image" placeholder="Link Image" class="border-b border-black text-xl w-100">
                </div>
                    <div class="pl-6 pt-5">
                    <input type="number" name='price' placeholder="Price" class="border-b border-black text-xl w-100">
                    <div class="pl-1 pt-5">
            ...
            <tr class="border-b border-gray-300 mt-4">
                <th class="md:p-5 sm:p-1 md:text-base text-xs">Id</th>
                <th class="md:p-5 p-1 md:text-base text-xs">Nama</th>
                <th class="md:p-5 p-1 md:text-base text-xs">Tanggal Terjual</th>
                <th class="md:p-5 p-1 md:text-base text-xs ">Harga</th>
            ...
            @if(empty($sold))
                <p>No Record</p>
            @endif
            @foreach ($sold as $single_product)
                
           
                <td class="text-center">{{$single_product->id}}</td>
                <td class="p-1 text-center text-xs md:text-xs">{{ $single_product->name }}</td>
                <td class="p-1 text-center text-xs md:text-xs">{{$single_product->created_at}}</td>
                <td class="p-1 text-left">Rp.{{ number_format("$single_product->price",2,",",".") }}</td>
                <td class="p-1 text-center pr-2"><a href="{{ route('DeleteConfirmSold', ['id' => $single_product->id]) }}"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 text-center text-red-500">
            ...
            <div class="absolute top-210 flex left-420 ">
                <a href="{{ route('NextSold',compact('count'))}}"><p class="text-blue-400">Next</p></a><p class="pl-5">{{$page}}</p><a href="{{ route('BackPage', compact('count')) }}"><p class="pl-5 pr-3 text-blue-400">Back</p></a>
            </div>
            </form>
            <form enctype="multipart/form-data" class="md:border-l md:border-gray-300 md:pl-20 pl-5 md:mr-85 md:mt-0 mt-20" >
                
                
                 <h1 class="md:p-6 md:text-base text-xs p-3 font-bold">High Selling Product</h1>
                    <table>
                        <tr class="border-b border-gray-300">
                            <th class="p-5">Image</th>
                            <th class="p-5">Product Name</th>
                            <th class="p-5">Sold</th>
                        </tr>
                    @foreach ($product as $high)
                    
                        <tr class="border-b border-gray-300">
                        <td class="p-3"><img src="{{ asset($high->image) }}" alt="" class="w-16 h-16 justify-self-center rounded-full items-center justify-items-center"></td>
                       <td class="p-3"> <h1>{{ $high->judul }}</h1></td>
                        <td class="p-3"><h1>{{ $high->qty }}</h1> </td>
                        </tr>
